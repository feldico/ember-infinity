...
export default class Infinity extends Service {
  ...
  */
  get store() {
    ...
    return (
      getOwner(this).lookup('service:store') || new (class extends Service {})()
    );
  }
  set store(value) {
    this._store = value;
  ...
  */
  ...
  pushObjects(infinityModel, queryObject) {
    if (checkInstanceOf(infinityModel)) {
      return infinityModel.pushObjects(queryObject);
    }
  }
  ...
  unshiftObjects(infinityModel, queryObject) {
    if (checkInstanceOf(infinityModel)) {
      return infinityModel.unshiftObjects(queryObject);
    }
  }
  ...
  replace(infinityModel, newCollection) {
    if (checkInstanceOf(infinityModel)) {
      ...
      infinityModel.replace(0, len, newCollection);
  ...
  */
  ...
  _calculateHeight(infinityModel) {
    if (typeof FastBoot === 'undefined') {
      let isScrollable = !!infinityModel._scrollable;
  ...
  _doUpdate(queryObject, infinityModel) {
    ...
    set(infinityModel, 'meta', queryObject.meta);
